...
export function useOrders(eventId, { usher_id } = {}) {
  ...
  useEffect(() => {
    ...
    async function fetchOrders() {
      ...
      if (usher_id) q = q.eq('usher_id', usher_id)
    ...
    const channelName = usher_id
      ? `orders-usher-${usher_id}`
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        // Fetch the new order with joined table info
        supabase
          .from('orders')
          .select('*, tables(table_name, table_number)')
          ...
          .then(({ data }) => {
            if (data) {
              const isRelevant = !usher_id || data.usher_id === usher_id
